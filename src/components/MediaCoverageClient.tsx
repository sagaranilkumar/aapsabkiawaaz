...
export default function MediaCoverageClient({ initialArticles }: MediaCoverageClientProps) {
  ...
  const resolveImgSrc = (imageSrc: string | undefined): string => {
    if (!imageSrc) return "/images/vizag-helmet-drive.jpg";
            ...
            <div className="flex items-center gap-3 w-full md:w-auto justify-end">
              <div className="bg-ngo-stone p-1 rounded-full flex gap-1 shadow-inner border border-ngo-border-light">
                <button
                  ...
                  className={`p-2 rounded-full transition-all cursor-pointer ${viewMode === "grid" ? "bg-white text-ngo-secondary shadow-sm" : "text-ngo-muted-light hover:text-ngo-muted"}`}
                ...
                <button
                  ...
                  className={`p-2 rounded-full transition-all cursor-pointer ${viewMode === "list" ? "bg-white text-ngo-secondary shadow-sm" : "text-ngo-muted-light hover:text-ngo-muted"}`}
                        ...
                        <article className="bg-ngo-card rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-ngo-border-light gold-glow flex flex-col h-full group relative focus-within:ring-2 focus-within:ring-ngo-secondary/50">
                          ...
                          <div className="p-6 flex-grow flex flex-col">
                            <div className="flex items-center justify-between text-[11px] text-ngo-muted-light font-semibold mb-3">
                              <span className="text-ngo-secondary uppercase tracking-wider">{source}</span>
                            ...
                            <h2 className="text-base font-bold text-ngo-primary group-hover:text-ngo-secondary transition-colors mb-2 leading-snug line-clamp-2">
                              {hasLink ? (
                                <a href={url} className="focus:outline-none" target={isExternal ? "_blank" : undefined} rel="noopener noreferrer">
                            ...
                            <p className="text-ngo-muted text-xs leading-relaxed mb-5 line-clamp-3">{description}</p>
                            ...
                            {hasLink ? (
                              <div className="flex items-center justify-between text-ngo-secondary font-bold text-xs mt-auto pt-2 border-t border-ngo-stone">
                                ...
                                {isExternal && <ExternalLink size={12} className="text-ngo-muted-light" />}
                              </div>
                            ) : (
                              <div className="flex items-center text-ngo-muted-light font-semibold text-xs mt-auto pt-2 border-t border-ngo-stone">
                          ...
                          <div className="flex-grow flex flex-col h-full w-full">
                            <div className="flex-grow">
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="inline-block text-ngo-secondary uppercase tracking-wider font-bold text-xs">{source}</span>
                              </div>
                              ...
                              <p className="text-ngo-muted text-sm leading-relaxed mb-4">{description}</p>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-auto pt-4 border-t border-ngo-border-light">
                              ...
                              {hasLink ? (
                                <div className="flex items-center text-ngo-secondary font-bold text-xs gap-1">
                                  Read Full Coverage
                                  <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
                                  {isExternal && <ExternalLink size={12} className="text-ngo-muted-light ml-1" />}
                                </div>
                              ) : (
                                <div className="flex items-center text-ngo-muted-light font-semibold text-xs">
                                  Full coverage coming soon
                                </div>
                              )}
                            </div>
