...
/** Newest item across media coverage + social posts, for the hero banner. */
...
/** A recent campaign card, derived from the newest media coverage entries. */
...
const IMPACT_STATS = [
  ...
  { number: 10000, suffix: "+", label: "Lives Impacted", icon: Heart },
  { number: 50, suffix: "+", label: "Campaigns Run", icon: Award },
  { number: 5, suffix: "+", label: "Cities Reached", icon: MapPin },
];

export default function HomeClient({ latest, campaigns }: HomeClientProps) {
  const latestTheme = latest ? themeFor(latest.category) : null;
  const LatestIcon = latestTheme?.icon;
      ...
      <section className="relative min-h-screen flex items-center pt-20 overflow-hidden bg-gradient-to-br from-ngo-cream via-ngo-background to-ngo-stone">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            ...
            {/* Latest banner — always the newest media or social entry */}
            {latest && latestTheme && LatestIcon && (
              <motion.div variants={fadeIn} className="mt-12">
                <Link
                  href={latest.href}
                  className="group mx-auto flex max-w-2xl items-center gap-4 rounded-2xl border border-ngo-border-light bg-ngo-card/80 p-3 pr-5 text-left shadow-lg backdrop-blur-sm transition-all hover:border-ngo-secondary/40 hover:shadow-xl focus:outline-none focus:ring-4 focus:ring-ngo-secondary/30"
                >
                  <div className="h-20 w-28 shrink-0 overflow-hidden rounded-xl bg-ngo-stone/40">
                    <img
                      src={latest.image}
                      alt=""
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="mb-1.5 flex flex-wrap items-center gap-2">
                      <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ring-1 ring-inset ${latestTheme.badge}`}>
                        <LatestIcon size={10} />
                        {latest.kind === "media" ? "Latest coverage" : "Latest post"}
                      </span>
                      <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-ngo-muted-light">
                        <Calendar size={11} />
                        {latest.date}
                      </span>
                    </div>
                    <p className="line-clamp-2 text-sm font-bold leading-snug text-ngo-primary transition-colors group-hover:text-ngo-secondary">
                      {latest.title}
                    </p>
                  </div>
                  <ArrowRight
                    size={18}
                    className="shrink-0 text-ngo-secondary transition-transform group-hover:translate-x-1"
                  />
                </Link>
              </motion.div>
            )}
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            {campaigns.map((campaign) => {
              const theme = themeFor(campaign.category);
              const CampaignIcon = theme.icon;
                ...
                >
                  ...
                  <div className="relative h-56 overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4">
                      <div className={`bg-white/90 backdrop-blur-sm p-2.5 rounded-xl shadow-sm ${theme.accentText}`}>
                        <CampaignIcon size={22} />
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Content */}
                  <div className="p-7">
                    <h3 className="text-xl font-bold text-ngo-primary mb-3 group-hover:text-ngo-secondary transition-colors line-clamp-2">
                      {campaign.title}
                    </h3>
                    <p className="text-ngo-muted text-sm leading-relaxed mb-5 line-clamp-3">
